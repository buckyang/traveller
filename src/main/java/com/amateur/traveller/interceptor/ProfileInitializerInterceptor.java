...
public class ProfileInitializerInterceptor extends HandlerInterceptorAdapter {
	static Logger logger = Logger.getLogger(ProfileInitializerInterceptor.class);
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if(session.getAttribute(Profile.PROFILE_KEY) == null){
			logger.debug("Initialize profile from[ address: " + request.getRemoteAddr() + ", " + request.getSession().getId() + "]" );
			Profile profile = new Profile();
			session.setAttribute(Profile.PROFILE_KEY, profile);
		}
		if(session.getAttribute(REDIRECT_ATTRIBUTES_KEY) != null){
			Map<String, Object> tempRedirectAttributes = (Map<String, Object>) session.getAttribute(REDIRECT_ATTRIBUTES_KEY);
